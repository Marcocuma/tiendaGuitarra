...
class RecycledViewAdapter(val context:Context, val listaGuitarra:List<Guitarra>, private val itemClickListener: OnGuitarraClickListener): RecyclerView.Adapter<BaseViewHolder<*>>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<*> {
        return GuitarrasViewHolder(LayoutInflater.from(context).inflate(R.layout.guitarra_row, parent, false))
    }
    interface OnGuitarraClickListener{
        fun onItemClick(marca:String,modelo:String)
        fun anadir(item:Guitarra)
    }
    //Si la funcion solo devuelve un dato
    override fun getItemCount(): Int = listaGuitarra.size

    override fun onBindViewHolder(holder: BaseViewHolder<*>, position: Int) {
        if(holder is GuitarrasViewHolder){
            holder.bind(listaGuitarra[position],position)
        }
    }

    inner class GuitarrasViewHolder(itemView: View):BaseViewHolder<Guitarra>(itemView){
        override fun bind(item: Guitarra, position: Int) {
            ...
            itemView.textViewPrecio.text = item.precio.toString()+" €"
            itemView.setOnClickListener {
                itemClickListener.onItemClick(item.marca,item.modelo)
            }
            itemView.button_anadir.setOnClickListener {
                itemClickListener.anadir(item)
            }
        }
    }
}